...
const ImageTicker: React.FC<ImageTickerProps> = ({
  ...
  return (
    <div className={clsx("relative w-full overflow-hidden bg-linear-to-r from-[#737373]/38 to-black/38 py-4 2xl:pt-[30px] 2xl:pb-[30px] border-[#00D962] border-t-2 border-b-2 2xl:border-t-6 2xl:border-b-6 ", className)}>
      <div className={clsx("flex w-max", animationClass, "space-x-0 gap-3 md:gap-5 2xl:gap-8 items-center")}>
        {[...images, ...images].map((image, index) => (
          <div
            key={index}
            className="flex items-center md:space-x-0 2xl:space-x-[10px] text-white text-sm 2xl:w-[360px] 2xl:h-[360px] rounded-xl 2xl:rounded-4xl"
          >
            <img
              src={image.src}
              alt={image.alt}
